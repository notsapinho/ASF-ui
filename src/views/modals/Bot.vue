  ...
  <main v-if="bot" class="main-container main-container--bot-profile">
    <div class="bot-profile" :class="[`status--${bot.status}`]">
      ...
      <div class="bot-profile__actions">
        <BotLink icon="wrench" :link="{ name: 'bot-config', params: { bot: bot.name } }" :title="$t('bot-fav-buttons-config')" />
        <BotLink icon="key" :link="{ name: 'bot-bgr', params: { bot: bot.name } }" :title="$t('bot-fav-buttons-bgr')" />
        <BotLink icon="lock" :link="{ name: 'bot-2fa', params: { bot: bot.name } }" :title="$t('bot-fav-buttons-2fa')" />

        <BotAction v-if="bot.paused && bot.active" icon="play" :title="$t('bot-resume-title', { bot: bot.name })" @click="resume" />
        <BotAction v-if="!bot.paused && bot.active" icon="pause" :title="$t('bot-pause-title', { bot: bot.name })" @click="pause" />

        <BotAction v-if="!bot.active" icon="power-off" :title="$t('bot-start-title', { bot: bot.name })" @click="start" />
        <BotAction v-if="bot.active" icon="power-off" :title="$t('bot-stop-title', { bot: bot.name })" @click="stop" />

        <BotLink icon="trash" :link="{ name: 'bot-delete', params: { bot: bot.name } }" :title="$t('bot-delete-title', { bot: bot.name })" class="pull-right" />
      </div>
    </div>

    <div class="bot-farming-info">
      <BotFarmingInfo :value="gamesRemaining" icon="gamepad" />
      <BotFarmingInfo :value="timeRemaining" icon="clock" />
      <BotFarmingInfo :value="cardsRemaining" icon="clone" />
    </div>
